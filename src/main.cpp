...
int main(){
    ...
    State *aliceStates = (State*) malloc(N_ROUND * sizeof(State));
    pair<double, Component> *bobMeasures = (pair<double, Component>*) malloc(N_ROUND * sizeof(pair<double, Component>));
    ...
    for(int i = 0; i<N_ROUND; i++){
        ...
        State aliceState = a.chooseState(generator);
        aliceStates[i] = aliceState;
        
        pair<double, Component> bobState = b.measure(c.trasmission(aliceState), generator);
        bobMeasures[i] = bobState;

        outputFile << aliceState.q.value << ',' << aliceState.p.value << ',' << aliceState.p.value << ";" 
                   << bobState.first << ";" << bobState.second << std::endl;

        std::cout << i << "--"<< bobState.first << "," << bobState.second << std::endl;
        
    }
    outputFile.close();

    vector<vector<double>> sifted = sifter(bobMeasures, aliceStates);
