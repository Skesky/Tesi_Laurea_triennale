...
Infine ver\`a utilizzato l'algoritmo \textit{sum-product}~\ref{al:sum-prod} di decodifica per ottenere una stima ${\underline{s}}$ della stringa $\underline s$ prodotta da Bob.

\begin{algorithm}[H]
\caption{: Riconciliazione}
\begin{algorithmic}[1]
...
\State \textbf{Step 5:} Estrazione del messaggi fittizio da parte di Alice e calcolo LLR
	\State $\textbf{r} \leftarrow messaggioFittzio(\textbf{m})$
