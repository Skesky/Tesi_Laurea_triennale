...
\titolo{Sviluppo di un simulatore per la crittografia quantistica a variabili continue}
...
\usepackage{algorithm, float}	%pacchetti utilizzati per la scrittura di pseudo-codice
\usepackage{algpseudocode}

\algnewcommand\algorithmicforeach{\textbf{for each}}			%definizione del for each loop
\algdef{S}[FOR]{ForEach}[1]{\algorithmicforeach\ #1\ \algorithmicdo}


\usepackage{caption}	%pacchetto per aggiungere le caption alle minipage
...
\capitolo{Descrizione quantistica di un segnale ottico}{capitolo_1}
\capitolo{Breve storia della QKD}{capitolo_2}
